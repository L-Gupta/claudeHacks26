...
export async function suggestMeetup(profileA, profileB) {
  ...
  try {
    ...
    const msg = await client.messages.create({
      ...
      messages: [{ role: 'user', content: `Student A: ${profileA.summary} (${profileA.major}, hobbies: ${(profileA.hobbies||[]).join(', ')})${profileA.transcript ? `\nVoice note: "${profileA.transcript}"` : ''}\nStudent B: ${profileB.summary} (${profileB.major}, hobbies: ${(profileB.hobbies||[]).join(', ')})${profileB.transcript ? `\nVoice note: "${profileB.transcript}"` : ''}\nSuggest a low-stakes meetup.` }],
    });
...
export async function suggestConnections(userProfile, neighbors) {
  ...
  try {
    ...
    const neighborList = neighbors.map((n, i) =>
      `${i+1}. Room ${n.room}: ${n.summary} (Major: ${n.major}, Hobbies: ${(n.hobbies||[]).join(', ')})${n.transcript ? ` Voice: "${n.transcript}"` : ''}`
    ).join('\n');

    const userVoice = userProfile.transcript ? `\nMy voice note: "${userProfile.transcript}"` : '';

    const msg = await client.messages.create({
      model: 'claude-sonnet-4-20250514',
      max_tokens: 500,
      system: 'You are a friendly matchmaker for UW-Madison dorm students. Given a student profile (including their voice note transcript if available) and their nearby neighbors, suggest the top connections with brief, warm, specific reasons referencing shared interests, personality cues from voice notes, or complementary qualities. Return ONLY a JSON array of objects with fields: room (string), reason (one sentence, warm and specific). No markdown.',
      messages: [{ role: 'user', content: `My profile: ${userProfile.summary} (Major: ${userProfile.major}, Year: ${userProfile.year}, Hobbies: ${(userProfile.hobbies||[]).join(', ')})${userVoice}\n\nNearby students:\n${neighborList}` }],
    });
    return JSON.parse(msg.content[0].text);
  } catch {
...
function mockSuggestions(user, neighbors) {
  const templates = {
    high: [
      (u, n, s) => `${n.name} in ${n.major} is also into ${s[0]} — you two would vibe at the Terrace on a Friday night.`,
      (u, n, s) => `You and ${n.name} both love ${s.join(' and ')}. That's not a coincidence, that's a friend waiting to happen.`,
      (u, n, s) => `${n.name} shares your ${s[0]} obsession and is a ${n.year} in ${n.major} — literally the hang you didn't know you needed.`,
    ],
    low: [
      (u, n) => `${n.name} studies ${n.major} and you're in ${u.major} — opposite worlds but that's what makes the best convos.`,
      (u, n) => `${n.name} has a totally different vibe (${n.hobbies?.[0]}, ${n.hobbies?.[1]}) which is exactly why you'd get along.`,
      (u, n) => `A ${u.major} major and a ${n.major} major on the same floor? The universe is trying to tell you something.`,
    ],
  };

  return neighbors.slice(0, 3).map(n => {
    const shared = (user.hobbies || []).filter(h => (n.hobbies || []).includes(h));
    const pool = shared.length > 0 ? templates.high : templates.low;
    const fn = pool[Math.floor(Math.random() * pool.length)];
    return { room: n.room, reason: fn(user, n, shared) };
  });
}
